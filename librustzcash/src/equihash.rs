...
impl Node {
    fn new(p: &Params, state: &blake2b_rfc::State, i: u32) -> Self {
...
fn initialise_state(n: u32, k: u32, digest_len: u8) -> blake2b_rfc::State {
    ...
    blake2b_rfc::Params::new()
        .hash_length(digest_len as usize)
        .personal(&personalization)
        .to_state()
}

fn generate_hash(base_state: &blake2b_rfc::State, i: u32) -> blake2b_rfc::Hash {
    let mut lei = [0u8; 4];
...
fn tree_validator(p: &Params, state: &blake2b_rfc::State, indices: &[u32]) -> Option<Node> {
    if indices.len() > 1 {
        let end = indices.len();
        let mid = end / 2;
